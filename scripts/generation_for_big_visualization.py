...
def run_simulation():
    """Запуск C++ частини через MPI"""
    print(f"--- Starting Surgical Simulation (Np: {NP_MIN}-{NP_MAX}) ---")
    try:
        # Переконайся, що шлях до EXECUTABLE вірний
...
def internal_plotter(csv_path, img_path):
    """Твоя логіка візуалізації, інтегрована як функція"""
    try:
        df = pd.read_csv(csv_path)
        if df.empty:
            return

        # Налаштування кроків (децимація)
        step_skip = 20
        ...
        # 2. ПРАВА ПАНЕЛЬ: Аналіз зближення (Логарифмічна шкала)
        ax2 = fig.add_subplot(122)
        v_p = 86.11   # Швидкість переслідувача
        dt = 0.01     # Часовий крок
        ...
        for pid in pursuers['id'].unique():
            ...
            for _, row in p_data.iterrows():
                ...
                if not step_esc.empty:
                    ...
                    # OY: Відстань до найближчого утікача
                    dx = step_esc['x'] - row['x']
                    dy = step_esc['y'] - row['y']
                    dz = step_esc['z'] - row['z']
                    min_dist = np.sqrt(dx**2 + dy**2 + dz**2).min()
                    distances.append(max(min_dist, 1e-3)) # Щоб не було log(0)
        ...
        plt.close(fig) # Обов'язково закриваємо, щоб не "з'їсти" всю оперативку
...
def generate_plots():
    """Пошук усіх CSV та виклик внутрішнього малювальника"""
    ...
    for root, dirs, files in os.walk(LOG_ROOT):
        for file in files:
            # Шукаємо наші хірургічні записи
...
if __name__ == "__main__":
    ...
    # 2. Генерація графіків (внутрішньою функцією)
    generate_plots()
